...
(function(){
  ...
  function renderStats(state){
    ...
    return `<section class="page-head"><div><h1>الإحصائيات</h1><p>تحليل آخر 30 يوماً، مع فصل الإتقان عن دقة المحاولات وجودة البيانات.</p></div></section>
      ...
      <div class="grid grid-2" style="margin-top:16px"><section class="card"><h3 style="margin-top:0">الأداء حسب المهارة</h3>${skills.map(x=>performanceBar(x)).join("")}</section><section class="card"><h3 style="margin-top:0">نشاط آخر 7 أيام</h3><df-activity-chart days="${DF.esc(JSON.stringify(days))}"></df-activity-chart></section></div>
  ...
  /* بطاقة قياس بلا أيقونة — تستخدمها خلاصة الجلسة وصف تدقيق البيانات.
     تُعرض القيمة كما تصل، لأن كل موضع استدعاء له تنسيقه الأصلي:
     خلاصة الجلسة تعرض الأرقام كما هي، بينما التدقيق ينسّقها محلياً. */
  function metricTile(value,label){return `<df-stat-tile value="${DF.esc(String(value))}" label="${DF.esc(label)}"></df-stat-tile>`;}
  ...
  function performanceBar(x){return `<df-skill-bar label="${DF.esc(DF.skillLabel(x.skill))}" accuracy="${x.accuracy}" attempts="${DF.esc(x.attempts.toLocaleString("ar-EG")+" محاولة")}" detail="${DF.esc("إتقان البطاقات "+x.value+"% · متوسط "+formatDuration(x.avgMs))}"></df-skill-bar>`;}
...
function renderQuestion(state,q){
    ...
    if(q.choices)body=`<df-choice-list choices="${DF.esc(JSON.stringify(q.choices.map(c=>({id:c.id,label:c.label}))))}" ${result?"revealed":""} correctid="${DF.esc(q.correctId??"")}" chosenid="${DF.esc(state.lastChoice??"")}"></df-choice-list>`;
      ...
      body=`<df-answer-input lang="${lang}" placeholder="${placeholder}" ${arabic?"arabic":""} ${result?"disabled":""}></df-answer-input><df-answer-actions ${result?"hasresult":""} ${q.usedHint?"usedhint":""}></df-answer-actions>`;
    }
